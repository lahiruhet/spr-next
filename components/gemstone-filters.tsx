...
const stoneTypes = [
  ...
  { name: "Sapphire", color: "#2563eb", selected: true },
...
export function GemstoneFilters() {
  const [priceRange, setPriceRange] = useState([0, 100000])
  const [caratRange, setCaratRange] = useState([1, 100])
        ...
        <div className="grid grid-cols-4 md:grid-cols-8 gap-4 justify-items-center">
          {stoneTypes.map((stone) => (
            <div key={stone.name} className="flex flex-col items-center gap-2">
              <div
                className={`w-10 h-10 rounded-sm ${stone.selected ? "ring-2 ring-offset-2 ring-[#1a1a1a]" : ""}`}
                style={{
